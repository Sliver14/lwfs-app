...
function Posts() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try{
        const response = await axios.get(`${apiUrl}/posts/postpage`);
      ...
      <div className='flex flex-col w-[98%] shadow-lg' >
        ...
        {listOfPosts.map((post) => ( 
          <div key={post.id} className='flex flex-col text-sm md:text-xs px-1 border-2 rounded-md border-gray-100  hover:border-[0.5px] hover:border-lw_gray hover:rounded-lg transition transform duration-150 ease-out'>
              <div className='flex flex-col h-[200px] w-[98%] mt-3' >
                <img className='flex h-full w-full rounded-lg object-cover transition tranform duration-200 ease-out lg:hover:scale-105' src={post.postPhoto}  alt=''/>
              </div>
